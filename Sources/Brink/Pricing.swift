...
final class PriceTable: ObservableObject {
    ...
    static let fileURL: URL = {
        ...
        return base.appendingPathComponent("Codenotch/brink/prices.json")
    }()
...
/// edit (Application Support/Brink/plans.json, seeded from the bundle) or point
...
final class PlanCatalog: ObservableObject {
    ...
    static let fileURL: URL = {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        return base.appendingPathComponent("Codenotch/brink/plans.json")
    }()
