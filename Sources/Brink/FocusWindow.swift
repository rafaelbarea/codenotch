...
struct FocusPane: View {
    ...
    private var stats: some View {
        HStack(spacing: 28) {
            stat(L10n.t("Focus time"), TimelinePane.duration(total))
            stat(L10n.t("Blocks"), "\(blocks.count)")
            if period != .day {
                let days = max(1, Set(blocks.map { cal.startOfDay(for: $0.start) }).count)
                stat(L10n.t("Per active day"), TimelinePane.duration(total / Double(days)))
            }
            if focus.isActive {
...
struct BlockRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            RoundedRectangle(cornerRadius: 2).fill(BrinkColors.violet).frame(width: 3, height: 26)
            ...
            Text(TimelinePane.duration(block.seconds))
