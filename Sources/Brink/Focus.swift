...
final class FocusStore: ObservableObject {
    ...
    static let fileURL: URL = {
        ...
        return base.appendingPathComponent("Codenotch/brink/focus-log.json")
    }()
    ...
    private func tick() {
        ...
        if !notifiedTarget, elapsed >= Double(targetMinutes * 60) {
            notifiedTarget = true
            BrinkNotify.post(title: L10n.t("Focus block done"), body: taskName ?? L10n.t("Time's up."))
            persistActive()
        }
        let due = Int((elapsed - Self.longAfter) / 3600) + 1
        if elapsed >= Self.longAfter, due > longAlerts {
            longAlerts = due
            BrinkNotify.post(title: L10n.t("Still in focus after \(Int(elapsed / 3600)) h"), body: taskName ?? L10n.t("Same task? Pause or stop it from the card."))
            persistActive()
        }
    }
...
enum BrinkNotify {
    static func post(title: String, body: String) {
        ...
        guard BrinkNotifications.usesMac else { BrinkNotifications.alert(title: title, body: body, sound: BrinkNotifications.focusSound); return }
        ...
        center.getNotificationSettings { settings in
            let deliver = {
                ...
                center.add(UNNotificationRequest(identifier: "brink-\(Int(Date().timeIntervalSince1970))", content: content, trigger: nil))
            }
