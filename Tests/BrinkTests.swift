...
/// The Brink features' seams into Codenotch: the tasks ring is a provider,
/// the hover card reserves room for the cost rows, credit-based Codex seats
/// get a Credits window, and the money math anchors on the allowance.
final class BrinkTests: XCTestCase {
    ...
    /// The tasks provider always answers with a Today window, even before the
    /// task app has been read (0 of 0), so the ring exists from the first poll.
    ...
    func testCardHeightGrowsWithCostRows() {
        ...
        let withRows = NotchLayout.cardHeight(windowCount: 2, brinkCostRows: 3)
        ...
        XCTAssertEqual(NotchLayout.cardHeight(windowCount: 2, brinkCostRows: 0), base)
    }
